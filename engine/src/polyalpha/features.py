...
class FeatureEngine:
    ...
    @staticmethod
    def _at(points: list[PricePoint], lookback: int) -> float:
        if len(points) <= lookback:
            return points[0].mid if points else 0.5
        return points[-1 - lookback].mid

    def build(self, markets: list[Market], books: dict[str, Book]) -> dict[str, FeatureVector]:
        now = time.time()
        ...
        for market_id, (market, yes_book, no_book, mid) in usable.items():
            ...
            output[market_id] = FeatureVector(
                ...
                imbalance=imbalance,
                return_1=mid - self._at(points, 1),
                return_3=mid - self._at(points, 3),
                return_10=mid - self._at(points, 10),
                rolling_mean=mean,
                zscore=zscore,
                volatility=vol,

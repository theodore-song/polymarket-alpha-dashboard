    ...
    <div className="drawer-backdrop" role="presentation" onMouseDown={close}>
      <aside className="agent-drawer" role="dialog" aria-modal="true" aria-label={`${agent.name} portfolio`} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <section className="drawer-section">
          ...
          {trades.length ? <div className="compact-list">{trades.slice(0, 8).map((trade) => <div key={trade.id}><div><strong>{trade.side} {trade.outcome} · {number.format(trade.shares)}</strong><span>{marketLabel(snapshot, trade.market_id)}</span></div><strong>{trade.price.toFixed(3)}</strong></div>)}</div> : <p className="empty-copy">This agent correctly stayed flat; no signal cleared costs and risk gates.</p>}
  ...
  return (
    <main>
      ...
      {view === 'methodology' && <section className="page-section methodology-page">
        ...
        <div className="method-grid"><article><span>01</span><h2>All-market observation</h2><p>Every open, active, order-accepting binary Polymarket market enters each cycle. A market must have usable books for both outcomes before it can produce a fill.</p></article><article><span>02</span><h2>Executable edge only</h2><p>Agents compare probability estimates with actual bid/ask prices. Taker fees, spread, liquidity, and minimum order size are subtracted before risk is authorized.</p></article><article><span>03</span><h2>Conservative accounting</h2><p>Holdings are marked at liquidation bids. Passive maker orders fill only after a later snapshot crosses or moves through the quote. Rewards and rebates are not credited.</p></article><article><span>04</span><h2>Survival first</h2><p>Fractional Kelly is capped at 2% per market, 10% per event, and 40 active markets. A 20% agent drawdown prevents new risk.</p></article></div>
